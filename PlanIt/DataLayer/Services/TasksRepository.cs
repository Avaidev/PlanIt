...
public class TasksRepository : IObjectRepository<TaskItem>
{
    ...
    public async Task<List<Node>> GetNodesForScheduledWithCategories(IEnumerable<Category> enumerable)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            {
                case "Today":
                    foreach (var task in await _taskRepo.FindManyAsync(t => Utils.CheckDateForToday(t.CompleteDate) &&  Utils.CheckDateForScheduled(t.CompleteDate) && !t.IsDone))
                    {
                ...
                case "Tomorrow":
                    foreach (var task in await _taskRepo.FindManyAsync(t => Utils.CheckDateForTomorrow(t.CompleteDate) && !t.IsDone, "tomorrow"))
                    {
                        task.CategoryObject = categories.FirstOrDefault(c => c.Id == task.Category);
                        node.Tasks.Add(task);
                    }
                ...
                case "Others":
                    foreach (var task in await _taskRepo.FindManyAsync(t => Utils.CheckDateForLater(t.CompleteDate) && !t.IsDone))
                    {
                        task.CategoryObject = categories.FirstOrDefault(c => c.Id == task.Category);
                        node.Tasks.Add(task);
                    }
